...
to setup
  ...
  create-cars Nb-agents [set heading 90 set shape "car" set color red
    if Initial-position = "Random" [move-to one-of patches with [pycor = 0 and not any? cars-here]]
    if Initial-position = "Jam" [set xcor who]]
...
to move
  ...
  ask cars [
    ...
    if model = "FVDM" [
      set acc (V distance pred - speed) / Tr + ([speed] of pred - speed) / Tr2
      set speed speed + dt * acc
    ]
